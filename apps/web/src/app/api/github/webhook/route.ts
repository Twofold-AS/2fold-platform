...
function verify(sigHeader: string | null, payload: string) {
  ...
  try { return timingSafeEqual(Buffer.from(sigHeader), Buffer.from(digest)); } catch { return false; }
}
...
export async function POST(req: Request) {
  ...
  // repository_dispatch → twofold-upgrade
  if (event === "repository_dispatch" && payload.action === "twofold-upgrade") {
    ...
    if (owner && repo && deps && typeof deps === "object") {
      ...
      const url = await openUpgradePR({ owner, repo, title: t, body, files: [{ path: "package.json", content: updated, message: t }] });
